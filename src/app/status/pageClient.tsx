...
import { ChevronDown, Clock3, HeartPulse, ShieldAlert, ShieldCheck, ShieldX, TimerReset, Watch } from 'lucide-react'
    ...
    return (
        ...
        >
            <div className='flex items-center justify-between gap-3'>
                ...
                <ChevronDown className={`h-4 w-4 shrink-0 text-(--color-text-discreet) transition-transform ${open ? 'rotate-180' : ''}`} />
            </div>
            ...
            <div className='mt-3 grid gap-3 800px:grid-cols-[minmax(0,1fr)_14rem]'>
                ...
                <div className='flex items-end justify-start overflow-x-auto 800px:justify-end'>
                    <div className='flex items-end gap-1 pb-1'>
                        {service.bars.slice(0, 30).toReversed().map((bar, index) => (
                            <div
                ...
                <div className='mt-3 rounded-lg bg-(--color-bg-body) p-4'>
                                ...
                                {service.checks.map((check) => (
                                    ...
                                    >
                                        <div className='flex flex-wrap items-center justify-between gap-2'>
                                            ...
                                            <Badge
                                                text={check.status ? text.statuses.healthy : text.statuses.issues}
                                                variant={check.status ? 'success' : 'danger'}
                                            />
                                        </div>
                    ...
                    <div className='mt-4 border-t border-(--color-border-default) pt-4'>
                        <div className='flex items-center justify-between'>
                            ...
                            {getCertStatus(service.certificate)}
                        </div>
                        {service.certificate ? (
...
function getCertStatus(certificate: MonitoringCertificate | undefined) {
    if (!certificate || !certificate.valid) return <ShieldX className='h-4 w-4 text-red-400' />
    if (certificate.subjectCN === certificate.issuer.cn) return <ShieldAlert className='h-4 w-4 text-orange-400' />
